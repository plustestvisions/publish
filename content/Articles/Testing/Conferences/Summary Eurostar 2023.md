---
sticker: emoji//1f302
tags: EuroSTARConf
draft: false
created: 2023-06-21T10:36
updated: 2025-01-10T17:35
title: Summary Eurostar 2023 Antwerp
aliases:
  - Wednesday 08/01
---
## The main take aways from Eurostar:
- It is all about the people
- Invest in your team to get collaborating more and better
- Watch the language you use
- Active listening is very important to know what the problem of your colleague(s) is/are then search for common problems and work on it together.
- Know your environment around your team and know who can help you
- Critical thinking is essential

## Critical Thinking
Definitions:
**Inference** is something we treat as true based on evidence
**assumption** is something that we treat as true even though we have insufficient evidence
**premise** is an assumption that begins a chain of reasoning. All logic is based on premises

Testers question assumptions & premises and gather data for better inferences
> [!seealso]
>  [[publish/Articles/Testing/Conferences/Critical Thinking Skills for Testers|Critical Thinking Skills for Tester]] talk from Michael Bolton


## Accessibility
Automated checks cannot find all errors only 57%
What do automated accessibility test find:
- Document structure
- labels
- invalid html
- color contrast
- Buttons must have text
- Images must have alternative text
- Fields must have a label
- Missing language attribute

## Teams related

Healthy team culture: 
- trust
- cooperation
- safety
- risk taking support
- accountability
- equity

A team should have different types of intelligence to become a better performing team:
### *Different types of intelligence:*
- Maths/Logical
- Linguistic
- Bodily/Kinesthetic
- Interpersonal
- Spatial/Dimensional
- Musical
- Natural
- Intrapersonal

We need all eight types of intelligence to be a genial team, that means communication is key within the team and outside the team (stakeholders). All our ideas have social and ethical implications. So there are 10 behaviors we like to see within team members, the most important is already mentioned is communication, 
> [!seealso] 
> >  [[publish/Articles/Testing/Conferences/10 Behaviours of Effective Employees]]

Sometimes you have to use persuasion to get the necessary information and this is OK..., but it does not always work - 
> [!seealso] 
> > [[publish/Articles/Testing/Conferences/Moving Cheese The Art of Persuasion for Testers]]


## Testing
#### Purpose of testing
Purposeful Testing is not just about what we test, but why and how we test it and the impact our testing has." 

> [!seealso]
>  > [[publish/Articles/Testing/Conferences/Testing with Purpose How Stories, Values and Kindness Make Testing Better]]

- stories give your testing work context, relevance and meaning
- Values guide our testing actions, decisions, and relationships
- Kindness is one gift anyone can give

How
- Start with why
- implement values in your daily work
- be kind
- set purposeful goals
- communicate and reflect

#### Architecture
Thinking on architecture is greatly simulated using this nice game. Came to know I have should played this game more with my nephew ;-)
At first I was convinced your first design should be simple, but actually there was one team that had a very nice compact and simple design in the first round, but after round 2 they transformed it into a really impressive complex system too. In the first round I noticed that all teams (except one) where designing a similar system/solution. The next round introduced new techniques a booster, a second level and in the last session some more complexity is added. Like in real life it becomes more difficult to integrate new stuff into your initial architecture. So in the starting phase I think you should consider as much different scenario's you know and also need to be aware of weaknesses and strong points too. Be realistic you can not foresee everything. 

#### Process Improvements
- Quality awareness is a continuous activity where this is important:
- Understanding and commitment is key
- Change takes longer than expected
- Commitment and compliance will degrade
- Continuous activity
- Importance of control (quality checks)
- It’s all in the mix


